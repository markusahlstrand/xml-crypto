...
import * as signatureAlgorithms from "./web-crypto-signature-algorithms";
import * as isDomNode from "@xmldom/is-dom-node";

export class WebCryptoSignedXml {
  idMode?: "wssecurity";
  idAttributes: string[];
  /**
  ...
  getKeyInfoContent = WebCryptoSignedXml.getKeyInfoContent;
  getCertFromKeyInfo = WebCryptoSignedXml.getCertFromKeyInfo;
  ...
  constructor(options: SignedXmlOptions = {}) {
    ...
    this.getCertFromKeyInfo = getCertFromKeyInfo ?? WebCryptoSignedXml.noop;
  ...
  enableHMAC(): void {
    ...
    this.getKeyInfoContent = WebCryptoSignedXml.noop;
  }
   ...
   * @throws TypeError If the xml can not be parsed.
   */
  computeSignature(xml: string, callback: ErrorFirstCallback<WebCryptoSignedXml>): void;
  ...
  computeSignature(
    ...
    callback: ErrorFirstCallback<WebCryptoSignedXml>,
  ): void;

  computeSignature(
    xml: string,
    options?: ComputeSignatureOptions | ErrorFirstCallback<WebCryptoSignedXml>,
    callbackParam?: ErrorFirstCallback<WebCryptoSignedXml>,
  ): void {
    let callback: ErrorFirstCallback<WebCryptoSignedXml>;
    if (typeof options === "function" && callbackParam == null) {
      callback = options as ErrorFirstCallback<WebCryptoSignedXml>;
      options = {} as ComputeSignatureOptions;
    } else {
      callback = callbackParam as ErrorFirstCallback<WebCryptoSignedXml>;
  ...
  getCanonXml(
    ...
    options.defaultNsForPrefix =
      options.defaultNsForPrefix ?? WebCryptoSignedXml.defaultNsForPrefix;
